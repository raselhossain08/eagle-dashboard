...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!accessToken) {
      console.log('❌ No access token found in global search API');
      return NextResponse.json(
    ...
    const response = await fetch(
      `${BACKEND_API_URL}/search/global?${queryParams}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { 
          error: errorData.message || 'Failed to perform global search',
